...
useEffect(() => {
  ...
  const start = async () => {
    try {
      const response = await fetch(`/api/block/get-last`);
      ...
      `;
      //console.log("WS Endpoint:", process.env.REACT_APP_INDEXER_WS_ENDPOINT);
      const client = createClient({
        url: process.env.REACT_APP_INDEXER_WS_ENDPOINT
      });
